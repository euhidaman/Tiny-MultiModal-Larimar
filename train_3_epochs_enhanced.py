...
class MultimodalVisualizationCallback(Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        if trainer.current_epoch % self.log_every_n_epochs == 0:
            ...
            print(
                f"✅ Visualizations completed for Epoch {trainer.current_epoch}")
    ...
    def log_embedding_visualizations(self, trainer, pl_module):
                ...
                try:
                    ...
                    if 'text_latent' in outputs:
                        text_embeddings.append(
                            outputs['text_latent'].cpu().numpy())
                    if 'vision_latent' in outputs:
                        vision_embeddings.append(
                            outputs['vision_latent'].cpu().numpy())
                    if 'multimodal_latent' in outputs:
                        multimodal_embeddings.append(
                            outputs['multimodal_latent'].cpu().numpy())

                    # Decode captions for labeling
                    # Limit to avoid memory issues
                    for input_ids in batch['input_ids'][:10]:
                        caption = pl_module.tokenizer.decode(
        ...
        self._create_embedding_plots(
            text_emb, vision_emb, multimodal_emb, captions, trainer.current_epoch)
    ...
    def _create_embedding_plots(self, text_emb, vision_emb, multimodal_emb, captions, epoch):
        ...
        try:
            ...
            # Text embeddings
            scatter1 = ax1.scatter(text_pca[:, 0], text_pca[:, 1], alpha=0.6, c=range(
                len(text_pca)), cmap='viridis')
            ...
            scatter2 = ax2.scatter(vision_pca[:, 0], vision_pca[:, 1], alpha=0.6, c=range(
                len(vision_pca)), cmap='plasma')
            ...
            scatter3 = ax3.scatter(multimodal_pca[:, 0], multimodal_pca[:, 1], alpha=0.6, c=range(
                len(multimodal_pca)), cmap='coolwarm')
            ...
            plt.tight_layout()
            wandb.log(
                {f"embeddings/pca_comparison_epoch_{epoch}": wandb.Image(fig)})
            plt.close()
    ...
    def log_multimodal_alignment(self, trainer, pl_module):
                ...
                try:
                    ...
                    if 'text_latent' in outputs and 'vision_latent' in outputs:
                        ...
                        alignment = torch.mean(
                            torch.abs(text_emb - vision_emb), dim=1)
        ...
        if similarities:
            ...
            ax1.set_ylabel('Frequency')
            ax1.set_title(
                f'Text-Vision Similarity (Epoch {trainer.current_epoch})')
            ax1.axvline(np.mean(similarities), color='red', linestyle='--',
                        label=f'Mean: {np.mean(similarities):.3f}')
            ax1.legend()

            # Alignment distribution
            ax2.hist(alignments, bins=30, alpha=0.7,
                     edgecolor='black', color='orange')
            ax2.set_xlabel('L1 Distance')
            ax2.set_ylabel('Frequency')
            ax2.set_title(
                f'Text-Vision Alignment (Epoch {trainer.current_epoch})')
        ...
        if hasattr(pl_module.model, 'memory') and pl_module.model.memory is not None:
            try:
                ...
                if hasattr(memory, 'code_memory'):
                    ...
                    utilization_rate = np.mean(memory_norms > 0.1)
                    ...
                    ax1.set_title(
                        f'Memory Slot Utilization (Epoch {trainer.current_epoch})')

                    # Memory heatmap
                    subset_size = min(50, memory_state.shape[0])
                    dim_size = min(50, memory_state.shape[1])
                    im = ax2.imshow(
                        memory_state[:subset_size, :dim_size], cmap='viridis', aspect='auto')
                    ax2.set_xlabel('Memory Dimension')
                    ax2.set_ylabel('Memory Slot')
                    ax2.set_title(
                        f'Memory State Heatmap ({subset_size}x{dim_size})')
        ...
        with torch.no_grad():
            for batch in val_dataloader:
                ...
                try:
                    ...
                    if 'text_latent' in outputs and 'vision_latent' in outputs:
                        # First 4 samples
                        text_emb = outputs['text_latent'][:4]
                        vision_emb = outputs['vision_latent'][:4]

                        # Compute attention-like scores
                        attention_scores = torch.matmul(
                            text_emb, vision_emb.transpose(-2, -1))
                        attention_scores = F.softmax(attention_scores, dim=-1)

                        # Plot attention matrix
                        fig, ax = plt.subplots(figsize=(10, 8))
                        im = ax.imshow(attention_scores.cpu(
                        ).numpy(), cmap='Blues', aspect='auto')
                        ax.set_xlabel('Vision Features')
                        ax.set_ylabel('Text Features')
                        ax.set_title(
                            f'Text-Vision Attention Patterns (Epoch {trainer.current_epoch})')
                        plt.colorbar(im, ax=ax)

                        wandb.log(
                            {f"attention/text_vision_epoch_{trainer.current_epoch}": wandb.Image(fig)})
                        plt.close()
    ...
    def log_learning_progression(self, trainer, pl_module):
        ...
        if len(self.loss_history['epoch']) > 1:
            ...
            # Reconstruction loss
            ax2.plot(
                epochs, self.loss_history['reconstruction'], 'g-', linewidth=2)
            ...
            # KL losses
            ax3.plot(
                epochs, self.loss_history['kl_text'], 'r-', label='Text KL', linewidth=2)
            ax3.plot(epochs, self.loss_history['kl_multimodal'],
                     'orange', label='Multimodal KL', linewidth=2)
            ax3.set_xlabel('Epoch')
            ...
            # Memory loss
            ax4.plot(
                epochs, self.loss_history['memory_kl'], 'purple', linewidth=2)
            ...
            plt.tight_layout()
            wandb.log(
                {f"progression/loss_curves_epoch_{trainer.current_epoch}": wandb.Image(fig)})
            plt.close()
            ...
            if len(self.embedding_history['epoch']) > 1:
                ...
                drift = np.mean(np.linalg.norm(
                    current_text - previous_text, axis=1))
...
def create_enhanced_trainer():
    ...
    wandb_logger = WandbLogger(
        ...
        tags=["3-epochs", "visualizations",
              "multimodal", "embeddings", "enhanced"],
