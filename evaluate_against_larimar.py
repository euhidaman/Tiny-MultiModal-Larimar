...
class LarimarBenchmarkEvaluator:
    ...
    def evaluate_generation_quality(self) -> Dict[str, float]:
        ...
        # Compute metrics
        results["bleu_1"] = self.compute_bleu(generated_texts, reference_texts, n=1)
        results["bleu_2"] = self.compute_bleu(generated_texts, reference_texts, n=2) 
        results["bleu_4"] = self.compute_bleu(generated_texts, reference_texts, n=4)
        
        results["rouge_1"] = self.compute_rouge(generated_texts, reference_texts, "rouge1")
        results["rouge_2"] = self.compute_rouge(generated_texts, reference_texts, "rouge2")
        results["rouge_l"] = self.compute_rouge(generated_texts, reference_texts, "rougeL")
        
        results["bertscore_f1"] = self.compute_bertscore(generated_texts, reference_texts)
        results["bleurt"] = self.compute_bleurt(generated_texts, reference_texts)
        ...
        for metric, original_value in original_results.items():
            if metric in our_results:
                ...
                if "perplexity" in metric:
                    ...
                    improvement = (original_value - our_value) / original_value * 100
                else:
                    # Higher is better for accuracy/quality
                    improvement = (our_value - original_value) / original_value * 100
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate Tiny-MultiModal-Larimar against original Larimar")
    ...
    parser.add_argument("--wandb_project", type=str, default="larimar-evaluation", help="W&B project name")
    ...
    # Run evaluation
    evaluator = LarimarBenchmarkEvaluator(args.model_path, args.config_path, args.device)
