...
from src.modules.babylm_data import BabyLMDataModule
...
def setup_wandb_logging(config: dict) -> WandbLogger:
    ...
    api = wandb.Api()
    runs = api.runs(f"{config['logging']['wandb_entity']}/{config['logging']['wandb_project']}")
    existing_names = [run.name for run in runs if run.name.startswith("optimal-larimar")]
...
def main():
    parser = argparse.ArgumentParser(description="Train optimal Tiny-MultiModal-Larimar")
    ...
    print(f"W&B Run: https://wandb.ai/{config['logging']['wandb_entity']}/{config['logging']['wandb_project']}/runs/{logger.experiment.id}")
    print("\nNext steps:")
